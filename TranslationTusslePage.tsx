...
import { TRANSLATION_PAIRS } from '../constants';
import { TranslationPair } from '../types';

const shuffleArray = (array: any[]) => [...array].sort(() => Math.random() - 0.5);

export const TranslationTusslePage: React.FC = () => {
    const navigate = useNavigate();
    const [score, setScore] = useState(0);
    const [questions, setQuestions] = useState<TranslationPair[]>([]);
    ...
    useEffect(() => {
        setQuestions(shuffleArray(TRANSLATION_PAIRS));
    }, []);
    ...
    const handleAnswer = (e: React.FormEvent) => {
        ...
        const isCorrect = inputValue.trim().toLowerCase() === currentQuestion.french.toLowerCase();
    ...
    return (
        <div className="max-w-xl mx-auto animate-fade-in">
            <div className="flex justify-between items-center mb-6 bg-card-light dark:bg-card-dark p-4 rounded-xl shadow-lg">
                ...
                <div className="text-xl font-bold">Word: {currentQuestionIndex + 1} / {questions.length}</div>
            </div>
            
            <div className="bg-card-light dark:bg-card-dark p-8 rounded-2xl shadow-xl text-center">
                <p className="text-lg text-subtle-dark dark:text-subtle-light mb-2">Translate this word to French...</p>
                <h2 className="text-4xl font-bold mb-8">{currentQuestion.english}</h2>
                ...
                {feedback && (
                    <div className="mt-4 p-4 rounded-lg animate-pop-in" style={{ backgroundColor: feedback === 'correct' ? 'var(--color-success)' : 'var(--color-danger)' }}>
                        <p className="font-bold text-white">
                            {feedback === 'correct' ? 'Correct!' : `The correct translation is ${currentQuestion.french}.`}
                        </p>
                    </div>
                )}
            </div>
