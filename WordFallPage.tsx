...
export const WordFallPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        gameLoopRef.current = window.setInterval(() => {
            ...
            setProblems(prevProblems => {
                const updatedProblems = prevProblems.map(p => ({ ...p, y: p.y + (0.8 + level * 0.15) }));
    ...
    useEffect(() => {
        if (gameOver) return;
        const spawner = window.setInterval(createProblem, 4000 / (1 + level * 0.1));
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setInputValue(value);
        const targetProblemIndex = problems.findIndex(p => p.word.toLowerCase() === value.trim().toLowerCase());
        if (targetProblemIndex > -1) {
            playSound('correct');
            const newScore = score + 50 * level;
            setScore(newScore);
    ...
    return (
        <div className="flex flex-col items-center animate-fade-in">
            <h2 className="text-3xl font-bold mb-4">Word Fall</h2>
            <div className="w-full max-w-4xl bg-card-light dark:bg-card-dark p-4 rounded-2xl shadow-xl">
                ...
                >
                    ...
                    {problems.map(p => (
                        <div
                            key={p.id}
                            className="absolute bg-white dark:bg-slate-800 text-slate-900 dark:text-white py-2 px-4 rounded-lg shadow-md select-none"
                            style={{ left: p.x, top: p.y, transition: 'top 16ms linear', maxWidth: '250px' }}
                        >
                            {p.definition}
                        </div>
                    ))}
                 ...
                 <input
                    ...
                    className="w-full mt-4 px-4 py-3 text-2xl text-center bg-bg-light dark:bg-bg-dark border-2 border-border-light dark:border-border-dark rounded-lg focus:ring-2 focus:ring-primary-light focus:border-primary-light transition"
                />
            </div>
        </div>
    );
