...
export const JumlaSaziPage: React.FC = () => {
    ...
    const getNewWord = useCallback(async (diff: Difficulty) => {
        ...
        setUserSentence('');
        try {
            const word = await geminiService.generateJumlaSaziWord(diff);
            setWordToUse(word);
        } catch (e) {
            console.error(e);
            // Handle error, maybe go back to arcade
    ...
    const handleSubmit = async () => {
        ...
        try {
            const result = await geminiService.validateJumlaSaziSentence(userSentence, wordToUse);
            setFeedback(result);
            if (result.isValid) {
                playSound('correct');
        ...
        return (
            <div className="flex flex-col items-center justify-center h-full text-center animate-fade-in">
                <div className="bg-card-light dark:bg-card-dark p-8 rounded-2xl shadow-xl">
                    ...
                    <p className="text-subtle-dark dark:text-subtle-light mb-8">Use the given word to make a correct sentence.</p>
