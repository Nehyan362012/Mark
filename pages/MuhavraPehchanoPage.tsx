...
interface MuhavraChallenge {
    muhavra: string;
    meaning: string;
    decoys: string[];
}

export const MuhavraPehchanoPage: React.FC = () => {
    const navigate = useNavigate();
    const { playSound } = useContext(SoundContext)!;
    const [score, setScore] = useState(0);
    const [currentQuestion, setCurrentQuestion] = useState<MuhavraChallenge | null>(null);
    ...
    const getNextQuestion = useCallback(async () => {
        ...
        setFeedback(null);
        try {
            const challenge = await geminiService.generateMuhavraChallenge();
            setCurrentQuestion(challenge);
            setOptions([challenge.meaning, ...challenge.decoys].sort(() => Math.random() - 0.5));
         ...
         return (
            <div className="flex flex-col items-center justify-center h-full text-center animate-fade-in">
                <div className="bg-card-light dark:bg-card-dark p-8 rounded-2xl shadow-xl">
                    <h2 className="text-4xl font-bold mb-6">Muhavra Pehchano</h2>
                    <p className="text-subtle-dark dark:text-subtle-light mb-8">Match the idiom to its meaning.</p>
    ...
    if (isLoading && !currentQuestion) return (
        <div className="text-center p-20">
            ...
            <p className="mt-4 text-lg font-semibold">Loading new idiom...</p>
        </div>
    );
    ...
    return (
        <div className="max-w-3xl mx-auto animate-fade-in">
            ...
            <div className="bg-card-light dark:bg-card-dark p-8 rounded-2xl shadow-xl text-center" lang="ur" dir="rtl">
                <p className="text-lg text-subtle-dark dark:text-subtle-light mb-2">اس محاورے کا کیا مطلب ہے؟</p>
                <h2 className="text-4xl font-bold mb-8">{currentQuestion.muhavra}</h2>
