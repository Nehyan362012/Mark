...
// Reusable card components from the image
const ControlPanelCard: React.FC<{ title: string; description: string; icon: JSX.Element; onClick: () => void }> = ({ title, description, icon, onClick }) => (
    <button onClick={onClick} className="relative group bg-card-light dark:bg-card-dark rounded-2xl shadow-lg p-6 text-left overflow-hidden h-36 flex flex-col justify-between transition-all transform hover:-translate-y-1 hover:shadow-xl dark:hover:shadow-primary-dark/10">
        ...
        <div className="absolute -left-2 -bottom-2 text-primary-light/5 dark:text-primary-dark/5 text-7xl transition-transform duration-300 group-hover:scale-110 group-hover:rotate-[-6deg] group-hover:text-primary-light/20 dark:group-hover:text-primary-dark/20">
            {icon}
        </div>
    </button>
);
...
const ForYouCard: React.FC<{ subject: string; title: string; description: string; onClick: () => void }> = ({ subject, title, description, onClick }) => (
    <div className="bg-card-light dark:bg-card-dark rounded-2xl shadow-lg p-6">
        ...
        <button onClick={onClick} className="font-bold text-primary-light dark:text-primary-dark">Let's go!</button>
    </div>
);
...
export const HomePage: React.FC = () => {
    ...
    const studentControlPanel = [
        ...
        { title: "Math Arcade", description: "Test your skills with challenges.", icon: ICONS.puzzle, path: '/math-arcade' },
    ];
