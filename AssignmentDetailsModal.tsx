...
interface AssignmentDetailsModalProps {
    ...
    onSimulate: (assignmentId: string) => void;
}

export const AssignmentDetailsModal: React.FC<AssignmentDetailsModalProps> = ({ assignment, onClose, onGrade, onSimulate }) => {
    const [isSimulating, setIsSimulating] = useState(false);

    const handleSimulate = async () => {
        setIsSimulating(true);
        await onSimulate(assignment.id);
    ...
    const statusStyles = {
        ...
        graded: 'bg-green-200 text-green-700'
    };
                        ...
                        <div key={sub.studentId} className="flex items-center justify-between p-2 bg-slate-50 dark:bg-slate-800/50 rounded-lg">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                {sub.status === 'submitted' && (
                                    <button onClick={() => onGrade(sub)} className="px-3 py-1 text-xs font-semibold rounded-lg text-white bg-primary-light">Grade</button>
                                )}
                            </div>
                        </div>
                    ))}
...
interface CreateAssignmentModalProps {
    isOpen: boolean;
    onClose: () => void;
    onCreate: (dueDate: string, documentFile: File | null) => void;
    quiz: Quiz;
}

export const CreateAssignmentModal: React.FC<CreateAssignmentModalProps> = ({ isOpen, onClose, onCreate, quiz }) => {
    ...
    return ReactDOM.createPortal(
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
            <form onSubmit={handleSubmit} className="bg-card-light dark:bg-card-dark rounded-2xl shadow-xl p-6 w-full max-w-md animate-pop-in space-y-4" onClick={e => e.stopPropagation()}>
                <h2 className="text-xl font-bold">Assign Exam</h2>
                <p className="p-2 bg-slate-100 dark:bg-slate-800 rounded-lg font-semibold">{quiz.title}</p>
                <div>
                ...
                <div>
                    ...
                    <input type="file" onChange={handleFileChange} className="w-full text-sm file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-primary-light/10 file:text-primary-light hover:file:bg-primary-light/20" />
                    {documentPreview && <img src={documentPreview} alt="preview" className="mt-2 max-h-24 rounded" />}
                    {documentFile && !documentPreview && <p className="mt-2 text-sm p-2 bg-slate-200 dark:bg-slate-700 rounded">📄 {documentFile.name}</p>}
                </div>
                <button type="submit" disabled={!dueDate} className="w-full py-2 font-semibold bg-primary-light text-white rounded-lg disabled:opacity-50">Assign to Class</button>
            </form>
        </div>,
        document.getElementById('modal-container')!
    )
