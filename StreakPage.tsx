...
const CalendarDay: React.FC<{ day: number; isFilled: boolean; isToday: boolean }> = ({ day, isFilled, isToday }) => {
    const baseClasses = "w-full h-12 flex items-center justify-center rounded-lg transition-all duration-300";
    let stateClasses = 'bg-slate-100 dark:bg-slate-800 text-subtle-dark dark:text-subtle-light';

    if (isFilled) {
        stateClasses = 'bg-gradient-to-br from-yellow-400 to-orange-500 text-white font-bold shadow-lg transform scale-105';
    }
    if (isToday) {
        stateClasses += ' ring-2 ring-primary-light dark:ring-primary-dark';
    }

    return <div className={`${baseClasses} ${stateClasses}`}>{day}</div>;
};
