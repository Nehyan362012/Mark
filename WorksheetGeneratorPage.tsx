...
const WorksheetOutput: React.FC<{ worksheet: Worksheet; onCopy: (text: string) => void }> = ({ worksheet, onCopy }) => {
    ...
    return (
        <div className="animate-fade-in p-4 md:p-6 lg:p-8 space-y-6">
...
export const WorksheetGeneratorPage: React.FC = () => {
    ...
    const handleGenerate = async () => {
        ...
        if (!topic || !grade || !subject || selectedTypes.length === 0) {
            addToast("Please fill in all fields and select at least one question type.", '⚠️');
            return;
        }
        ...
        try {
            const result = await generateWorksheet(topic, grade, subject, numQuestions, selectedTypes);
            setOutput(result);
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 h-full max-h-[calc(100vh-8.5rem)]">
            {/* INPUT PANEL */}
            <div className="bg-card-light dark:bg-card-dark lg:rounded-l-2xl p-6 flex flex-col space-y-4 overflow-y-auto">
                ...
                <div>
                    ...
                    <input type="range" min="3" max="20" step="1" value={numQuestions} onChange={e => setNumQuestions(parseInt(e.target.value))} className="w-full h-2 bg-slate-200 dark:bg-slate-700 rounded-lg appearance-none cursor-pointer accent-primary-light dark:accent-primary-dark"/>
                </div>
                <div>
                        ...
                        {Object.keys(questionTypes).map(type => (
                            <label key={type} className="flex items-center gap-2 text-sm">
                                <input
                                    ...
                                    onChange={e => setQuestionTypes(prev => ({...prev, [type]: e.target.checked}))}
            ...
            {/* OUTPUT PANEL */}
            <div className="bg-bg-light dark:bg-bg-dark lg:rounded-r-2xl overflow-y-auto">
                ...
                {!isLoading && !error && !output && (
                     <div className="flex flex-col items-center justify-center text-center h-full text-subtle-dark dark:text-subtle-light p-8">
                        ...
                        <p className="mt-2 max-w-sm">Fill in the details on the left, and let our AI assistant craft a custom worksheet for your students.</p>
                    </div>
                )}
            </div>
        </div>
