...
const QuizCard: React.FC<{ quiz: Quiz; onPlay: () => void; onPublish: () => void; isTeacher: boolean; }> = ({ quiz, onPlay, onPublish, isTeacher }) => (
    <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-all duration-300 transform hover:scale-[1.02] hover:bg-slate-100 dark:hover:bg-slate-800">
        <div onClick={onPlay} className="flex-grow cursor-pointer">
            <h4 className="font-bold text-lg">{quiz.title}</h4>
            <p className="text-sm text-subtle-dark dark:text-subtle-light">{quiz.questions.length} Questions</p>
        </div>
            ...
            {userQuizzes.length > 0 ? (
                <div className="space-y-8">
                    {sortedSubjects.map((subject, index) => (
                        <div key={subject} className="bg-card-light dark:bg-card-dark rounded-3xl shadow-xl p-6 animate-fade-in-down" style={{animationDelay: `${index * 100}ms`}}>
                ...
                <div className="text-center py-16 px-6 bg-card-light dark:bg-card-dark rounded-3xl shadow-lg animate-pop-in">
