...
export const PresetQuizSetupPage: React.FC = () => {
    ...
    const handleQuizGeneration = async (sub: string, gr: string, num: number, top?: string) => {
        setIsLoading(true);
        const questions: QuizQuestion[] = await generatePresetQuiz(sub, gr, num, top);
        setIsLoading(false);
    ...
    const handleStart = async () => {
        ...
        if (showTimer && timerDuration > 0) {
           ...
           await handleQuizGeneration(subject, grade, count, topic);
        }
    };

    const handlePersonalizedQuiz = () => {
        // Simple logic: pick the subject the user rated lowest.
        // A more complex version could use quiz history.
        const proficiencies = user.proficiencies || {};
        const subjects = Object.keys(proficiencies);
        if (subjects.length === 0) {
            alert("Please complete your subject proficiencies in your profile to use this feature!");
            return;
        }

        const weakestSubject = subjects.reduce((a, b) => proficiencies[a] < proficiencies[b] ? a : b);
        handleQuizGeneration(weakestSubject, user.grade || GRADE_LEVELS[8], 10, `basics of ${weakestSubject}`);
    };
                    ...
                    <div className="space-y-6">
                        {user.role === 'student' && (
                            <div className="p-4 bg-primary-light/10 dark:bg-primary-dark/20 rounded-2xl text-center">
                                ...
                                <button onClick={handlePersonalizedQuiz} className="font-semibold text-white bg-primary-light dark:bg-primary-dark px-4 py-2 rounded-lg shadow hover:opacity-90">
